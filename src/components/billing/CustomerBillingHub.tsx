...
export const CustomerBillingHub: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (CustomerBillingEngine.getAllBillingDocuments(tenantId, companyId).length === 0) {
      // 1. Standard Invoice
      const inv1 = CustomerBillingEngine.createBillingDocument({
        tenantId,
        companyId,
        ...
        lines: [
          {
            sku: 'SKU-SERVERS-01',
            description: 'Enterprise Rack Servers (10 Units)',
            billedQuantity: 10,
            uom: 'EA',
            unitPrice: 2500,
      ...
      CustomerBillingEngine.postBillingDocument(inv1.id, 'finance.controller@am-enterprise.com', false);

      // 2. IFRS 15 Contract
      const ctr = CustomerBillingEngine.createIFRS15RevenueContract({
        tenantId,
        companyId,
        salesOrderId: 'SO-2026-BUNDLE-99',
        customerId: 'cust-101',
        customerName: 'Al-Ahram Commercial',
        totalTransactionPrice: 120000,
        currency: 'USD',
        performanceObligations: [
          {
            name: 'Core ERP Software Perpetual License',
            pobType: 'POINT_IN_TIME',
            standaloneSellingPrice: 90000,
            startDate: '2026-09-01',
            endDate: '2026-09-01'
          },
          {
            name: '36-Month Premium 24/7 SLA Support',
      ...
      // Generate Amortization Schedule & Recognize Initial Point-in-time
      CustomerBillingEngine.recognizePercentageOfCompletion(
        ctr.id,
        ctr.performanceObligations[0].id,
        100,
        'revrec.officer@am-enterprise.com'
      );
      CustomerBillingEngine.generateRevenueAmortizationSchedule(
        ctr.id,
        ctr.performanceObligations[1].id,
        36,
        2026,
        9
      );
      ...
      // 3. Milestone Plan
      CustomerBillingEngine.createMilestoneBillingPlan({
        tenantId,
        companyId,
        salesOrderId: 'SO-TURNKEY-CAIRO',
        customerId: 'cust-101',
        customerName: 'Al-Ahram Commercial',
        totalContractValue: 300000,
        currency: 'USD',
        stages: [
          { stageName: 'Phase 1: Mobilization & Blueprint', milestonePercentage: 25, retentionPercentage: 10, targetDate: '2026-09-30' },
          { stageName: 'Phase 2: Core Engineering', milestonePercentage: 35, retentionPercentage: 10, targetDate: '2026-12-15' },
          { stageName: 'Phase 3: Integration & UAT', milestonePercentage: 30, retentionPercentage: 10, targetDate: '2027-03-31' },
          { stageName: 'Phase 4: Final Go-Live Acceptance', milestonePercentage: 10, retentionPercentage: 0, targetDate: '2027-06-30' }
        ],
        performedBy: 'pm@am-enterprise.com'
      });

      // 4. Run Dunning Evaluation
